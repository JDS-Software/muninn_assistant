...
---@return string
function MnContext:get_file(relpath)
    local path_parts = { self.home_dir, relpath }
    return vim.fs.joinpath(unpack(path_parts))
end
...
local function new_context(fn_ctx)
    ...
    return setmetatable({ fn_context = fn_ctx, an_context = an_ctx, home_dir = plugin_root }, MnContext)
end
